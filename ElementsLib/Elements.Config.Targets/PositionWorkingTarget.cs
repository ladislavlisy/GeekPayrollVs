...
namespace ElementsLib.Elements.Config.Targets
{
    ...
    public class PositionWorkingTarget : ArticleGeneralTarget, ICloneable
    {
        public static string TARGET_POSITION_WORKING_EXCEPTION_RESULT_NULL_TEXT = "PositionWorkingTarget(5): Evaluate Results is not implemented!";

        public PositionWorkingTarget() : base((ConfigCode)ConfigCodeEnum.TARGET_POSITION_WORKING)
        {
            SourceValues = new PositionWorkingSource();
        }

        public PositionWorkingTarget(ISourceValues values) : this()
        {
        ...
        {
            return string.Format("PositionWorkingSource(TARGET_POSITION_WORKING, 5): { 0 }", message);
        }
        ...
        public override IEnumerable<ResultPack> EvaluateResults(HolderItem evalHolder, Period evalPeriod, IPeriodProfile evalProfile, IEnumerable<ResultPack> evalResults)
        {
            ...
            return ErrorToResults(TARGET_POSITION_WORKING_EXCEPTION_RESULT_NULL_TEXT);
        }

        public override object Clone()
        {
            PositionWorkingTarget cloneArticle = (PositionWorkingTarget)this.MemberwiseClone();
