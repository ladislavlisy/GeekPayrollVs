...
namespace ElementsLib.Elements.Config.Articles
{
    ...
    using SourceItem = Sources.TaxAdvanceOrdinarySource;
    ...
    public class TaxAdvanceOrdinaryArticle : GeneralArticle, ICloneable
    {
        ...
        public static string ARTICLE_DESCRIPTION_ERROR_FORMAT = "TaxAdvanceOrdinaryArticle(ARTICLE_TAX_ADVANCE_ORDINARY, 1020): {0}";

        public TaxAdvanceOrdinaryArticle() : base((ConfigRole)ConfigRoleEnum.ARTICLE_TAX_ADVANCE_ORDINARY)
        {
            SourceValues = new TaxAdvanceOrdinarySource();

            InternalEvaluate = TaxAdvanceOrdinaryConcept.EvaluateConcept;
        }

        public TaxAdvanceOrdinaryArticle(ISourceValues values) : this()
        {
            TaxAdvanceOrdinarySource sourceValues = values as TaxAdvanceOrdinarySource;

            SourceValues = CloneUtils<TaxAdvanceOrdinarySource>.CloneOrNull(sourceValues);
        }
        ...
        public TaxAdvanceOrdinarySource SourceValues { get; set; }

        public override void ImportSourceValues(ISourceValues values)
        {
            SourceValues = SetSourceValues<TaxAdvanceOrdinarySource>(values);
        }
        ...
        {
            TaxAdvanceOrdinaryArticle cloneArticle = (TaxAdvanceOrdinaryArticle)this.MemberwiseClone();
